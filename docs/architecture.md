...
The boundary is scoped to repositories that actually have an upstream, so a project with no `origin` remote configured - a shape `local-only` delivery permits - reports the skip and launches from its local default-branch base, while a configured remote that cannot be reached is still an unverifiable base that stops the spawn.
