...
`fm-spawn.sh` also owns the base-freshness boundary for every fresh ship and scout: no worker starts until its clean task worktree matches the current tip of that project's resolved default branch, and any unsafe or unverifiable base stops the spawn.
The boundary follows whichever base a project actually has, so a project with no `origin` remote configured - a shape `local-only` delivery permits - reports the skip and resets to the tip of its local default branch, while a configured remote that cannot be reached is still an unverifiable base that stops the spawn.
